...
class Rectangle:
    ...
    def contains(self, point: Point2D) -> bool:
        return (
            self.is_in_interval(point.x, self._lower_left.x, self._lower_left.x + self._dx)
            and self.is_in_interval(point.y, self._lower_left.y, self._lower_left.y + self._dy)
        )
